...
### v4.1 (Latest) - 智能装机与硬件适配
*   **硬件自适应**: 自动检测 OS/GPU/CUDA 版本，智能推荐 PyTorch/TensorFlow 的最佳版本 (GPU vs CPU)，并在安装时自动过滤不兼容的包。
*   **全家桶升级**: 新增“网络爬虫”套件，全面扩充深度学习/Web/数据科学套件的依赖包（如新增 PyTorch Lightning, Scrapy, FastAPI 等）。
*   **高度可定制**: “一键装机”支持可视化查看并勾选/取消特定依赖包，支持自定义环境名称，拒绝黑盒安装。
*   **UI 体验**: 优化 Web 界面交互，新增平滑过渡动画与硬件信息实时展示横幅。

### v4.0 - v4.0 核心能力补全
*   **一键环境初始化**: 新增 Deep Learning / Web Dev 模板，一键批量应用常用配置。
*   **插件系统**: 支持从 `plugins/` 目录加载并运行自定义 Python 脚本扩展功能。
*   **自动更新**: 启动时检查 GitHub Release 并提示新版本（若可识别仓库地址）。
